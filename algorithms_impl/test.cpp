//
// Created by anguangyan on 22-9-25.
//
#include <vector>
#include <algorithm>
#include <limits>
...
int main() {
    int job_exec_times[] = {3, 3, 5, 6, 7, 3, 1, 3, 4, 4};
    int n = sizeof(job_exec_times) / sizeof(int);
    int worker_num = 4;
    int *result = LoadBalancingInt(job_exec_times, n, worker_num);
    for (int i = 0; i < worker_num; ++i) {
        std::cout << result[i] << " ";
    }
    std::cout << std::endl;
    ...
    std::cout << std::endl;
    delete [] result;
    result = LoadBalancingGreedyInt(job_exec_times, n, worker_num);
    for (int i = 0; i < n; ++i) {
        std::cout << result[i] << " ";
    }
    std::cout << std::endl;
